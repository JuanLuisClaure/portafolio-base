...
admin.initializeApp({
  ...
  databaseURL: "https://cliente-temporal.firebaseio.com"
});



const ownerdb = admin.database().ref('owner/');
...
huespedSocketRoom.on('connection', (cliente) => {
  ...
  cliente.on('weareconnect', (interesadoinfo) => {
    ...
    clientekey.then((value) => {
      cliente.emit('nuevo', {
        socketdata: cliente.handshake,
        dbkey: clientekey.key
      })
    }).catch((err) => {
  ...
  cliente.on('updateroomid', (key) => {
    let update = clientesdb.child(`${key}`).update({
      0: cliente.id
    })
    update.then(() => {
      // console.log(cliente.id,'room update');
    }).catch((err) => {
      console.log('acavieneelformulariointereses');
    })
  })
...
OwnerSocketRoom.on('connection', (owner) => {
  ...
  owner.on('informacion:owner', (e, b) => {
    console.log('llego', e)
    ownerdb.child(e).update({
      "data": b
    })
  ...
  if (crear === true) {
    return clientesdb.once('value', function (data) {
      let d = new Promise((resolve, reject) => {
        ...
        console.log('error en labase de datos');
      })
      return d
    })
  } else if (crear === false) {
...
api.use(async (ctx, next) => {
  ...
  switch (p) {
    case '/':
      ...
      let chatcmpnt = riot.render(x_tag_cmpnt_chatcliente, {
        ownerinfo: datos.val()
      })
      // console.log(vrfcliente.visto)
      // console.log(usuario)        
      let vistacliente = await ctx.render('start', {
        a: ipper.ip,
        renderHere: chatcmpnt,
        ownerh2o: datos.val(),
        verifiSocket: vrfcliente.visto,
        dbkeyuser: vrfcliente.data,
        datoscliente: (usuario === 'nuevo') ? (["idroom", {
          "key": "value"
        }]) : usuario.child(vrfcliente.data).val()
      })
    ...
    case '/login':
      ...
      if (vrf.owner === false) {
        if (ctx.method === 'GET') {
          ...
          let css = await ctx.render('login', {
            renderHere: cpp
          })
    ...
    case (p.toString().match(re) || {}).input:
      ...
      if (vrfs.owner === false) {
        ...
        let componente = await riot.render(son.cpmnt, {
          data: datoss.val()
        });
        let cs = await ctx.render('dashboard', {
          renderHere: componente,
          data: datoss.val(),
          componente: son.cpmnt
        })
      }
